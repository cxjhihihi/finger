...
public class StringUtils {
    ...
    private static final String[] commonWords = new String[] { "a", "and",
        "as", "at", "be", "do", "i", "if", "in", "is", "it", "so", "the", "to" };
    ...
    /**
     * Pseudo-random number generator object for use with randomNumberString().
    ...
    /**
     * Array of numbers. Numbers appear in the list twice so that there is a
     * more equal chance that a number will be picked. We can use the array to
     * get a random number by picking a random array index.
     */
    ...
    private static final String convertEncoding(String srcString,
        String fromEncoding, String toEncoding)
        throws UnsupportedEncodingException {
        String encString = null;
        if (srcString != null) {
            byte[] b = srcString.getBytes(fromEncoding);
            encString = new String(b, toEncoding);
        }
        return encString;
    }

    public static int countSubString(String srcStr, String subStr) {
        int count = 0;
        for (int i = 0; i < srcStr.length(); i++) {
            if (srcStr.startsWith(subStr, i))
                count++;
        }
        return count;
    }

    public static final Hashtable<String, String> csvToHash(String in) {
        Hashtable<String, String> h = new Hashtable<String, String>();
        StringTokenizer st = new StringTokenizer(in, "\t");
        while (st.hasMoreTokens()) {
            String l = st.nextToken();
            int p = l.indexOf('=');
            String k = l.substring(0, p);
    ...
    public static final String decode(String srcString, String fromEncoding)
        throws UnsupportedEncodingException {
        return convertEncoding(srcString, fromEncoding, localEncoding);
    }

    public static final String decode(String srcString, String fromEncoding,
        String toEncoding) throws UnsupportedEncodingException {
        return convertEncoding(srcString, fromEncoding, toEncoding);
    }

    public static final String encode(String srcString, String toEncoding)
        throws UnsupportedEncodingException {
        return convertEncoding(srcString, localEncoding, toEncoding);
    }
    ...
    public static final String escapeData(char[] ch, int start, int length) {
    ...
    public static final String escapeForXML(String input) {
        /*
         * jive's version //Check if the string is null or zero length -- if so,
         * return //what was sent in. if (string == null || string.length() == 0
         * ) { return string; } char [] sArray = string.toCharArray();
         * StringBuffer buf = new StringBuffer(sArray.length); char ch; for (int
         * i=0; i<sArray.length; i++) { ch = sArray[i]; if(ch == '<') {
         * buf.append("&lt;"); } else if( ch == '>' ) { buf.append("&gt;"); }
         * else if (ch == '&') { buf.append("&amp;"); } else if (ch == '"') {
         * buf.append("&quot;"); } else { buf.append(ch); } } return
         * buf.toString();
         */
        ...
        for (int i = 0; i < input.length(); i++) {
             ...
             * http://www.xml.com/2000/05/10/conformance/reports
             * /report-xerces-nv.html
             */

            // XML Predefined Entiey: & < > ' "
            if (ch == '>' && i > 1) {
                //escape for CDATA section
    ...
    public static final String hashToCsv(Hashtable<String, String> h) {
        ...
        while (e.hasMoreElements()) {
            ...
            ob.append(k + "=");
            ob.append(escapeData(v.toCharArray(), 0, v.length()) + "\t");
        }
        return ob.toString();
    }
    ...
    public static final String randomNumString(int length) {
        ...
        if (randGen2 == null) {
            synchronized (initLock) {
                if (randGen2 == null) {
                    randGen2 = new Random();
                    //Also initialize the numbersAndLetters array
                    numbersAndLetters2 = ("0123456789" + "0123456789")
                        .toCharArray();
                }
            }
        }
        ...
        char[] randBuffer = new char[length];
        for (int i = 0; i < randBuffer.length; i++) {
            randBuffer[i] = numbersAndLetters2[randGen2.nextInt(19)];
        }
        return new String(randBuffer);
    }
    ...
    /**
     * split a string to strings'array with delim
     */
    public static final String[] splitString(String string, String delim) {
        int index = 0;
        String[] arr = null;
    ...
    public static final String[] splitStringLimit(String string, String delim) {
        int index = 0;
        String[] arr = null;
        String temp = null;
        ArrayList<String> list = new ArrayList<String>();
    ...
    public static final String[] splitString1(String string, String delim) {
        String[] strArr = null;
        StringTokenizer st = null;
        if (string == null || string.length() == 0) {
            return null;
        } else {
            st = new StringTokenizer(string, delim, false);
        }
    ...
    /** Added by zhou_yue. */
    public static String reform(String arg) {
        if (arg == null)
            return null;
        arg = arg.trim();
        if (arg.equals(""))
    ...
    public static final String unescapeData(char[] ch, int start, int length) {
        StringBuffer buf = new StringBuffer();
        for (int i = start; i < (start + length); i++) {
            if (ch[i] == '\\' && i != (start + length)) {
                switch (ch[i + 1]) {
                    case 't':
                        buf.append('\t');
                        break;
    ...
    /** Added by zhou_yue. Get current date of string. */
    ...
    public static String format2GBK(String oldValue) {
        if (oldValue == null) {
            return null;
        }
        try {
            String newValue = new String(oldValue.getBytes("iso8859-1"),
                "gb2312");
            return newValue;
